...
WITH not_null AS (
	SELECT *,
		COALESCE(product_size, '') as product_size_notnull,
		COALESCE(product_qty_type, 'unit') as product_qty_type_notnull
	FROM product)

SELECT 
product_name || ', ' || product_size_notnull || ' (' || product_qty_type_notnull || ')'
FROM not_null;
...
ranked AS (
  SELECT
    market_date,
    total_sales,
    DENSE_RANK() OVER (ORDER BY total_sales DESC) AS r_desc, -- highest = 1
    DENSE_RANK() OVER (ORDER BY total_sales ASC)  AS r_asc   -- highest = 1
  FROM sales_by_date
)
SELECT 'highest day'  AS which_day, market_date, total_sales
FROM ranked
WHERE r_desc = 1
UNION
SELECT 'lowest day' AS which_day, market_date, total_sales
FROM ranked
WHERE r_asc = 1
ORDER BY which_day, market_date;
...
VALUES(24,'Rhubarb Pie', '12"', 3, 'unit', CURRENT_TIMESTAMP);
